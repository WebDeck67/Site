                        ...
                        {items.map((item, idx) => (
                            ...
                            >
                                <a
                                    href={item.href}
                                    key={item.title}
                                    className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10 backdrop-blur-sm border border-white/20"
                                >
...
const FloatingDockDesktop = ({
    ...
    return (
        <motion.div
            ...
            className={cn(
                "mx-auto hidden h-16 items-end gap-4 rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 px-4 pb-3 md:flex",
                className,
            )}
        >
            {items.map((item) => (
                ...
                <AnimatePresence>
                    {hovered && (
                        <motion.div
                            ...
                            className="absolute -top-8 left-1/2 w-fit rounded-md border border-white/20 bg-white/10 backdrop-blur-sm px-2 py-0.5 text-xs whitespace-pre text-white"
                        >
                            {title}
                        </motion.div>
                    )}
